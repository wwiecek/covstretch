...
# Comparison withn lops bfs - 5 groups ----
load("results/opt-bfs-result5.Rdata")
#Change the file below, the 'variable' filter, and the argument in model_fd_static for deaths/infections
load("results/wip-nl-solutions5-I.Rdata")
...
for (D in unique(nl_d_seq)){
  ...
  res_nlopt_0 <- nlopt_d0[2:6,as.character(D)]
  eval_nlopt_1 <- nlopt_d1[1,as.character(D)]
  res_nlopt_1 <- nlopt_d1[2:6,as.character(D)]
