...
multi_year_run <- function(par, nperiods = 2) {
  ...
  for(i in 1:(nperiods-1)){
    ...
    # last_state["cumV1",] <- last_state["cumV2",] <- last_state["cumV",] <- 0*last_state["cumV1",]
    # last_state["cumV",] <- 0*last_state["cumV",]
    # simulate another period
    cpar <- list_modify(par, y0 = last_state, vstop = rep(1.2, 9))
    cy <- sr(cpar)
...
par <- list_modify(par, 
                   phi = rep(1/365, 9),
                   kappa1 = rep(1/365, 9))
multi_year_run(par, nperiods = 3) %>% plot_rcs(c("I", "P1", "S", "cumV"))
